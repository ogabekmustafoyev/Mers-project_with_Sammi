...
function accordion() {
  ...
  accordion.forEach((acc) => {
    acc.addEventListener("click", () => {
      ...
      const panel = acc.nextElementSibling; // Returns the first following element
...
function data() {
  ...
  function getTime(endtime) {
    const total = Date.parse(endtime) - Date.parse(new Date()), //returns the milliseconds from January 1, 1970.
      days = Math.floor(total / (1000 * 60 * 60 * 24)), // returns full number(5.1 && 5.9 == 6)
...
function form(modalTimer) {
  ...
  const postData = async (url, data) => {
    ...
    //  aync await (amal bajarilmaguncha kutib turish)
    return await res.json();
  };
